...
def show_prediction(models, scaler, label_encoder, label_encoders):
    ...
    if uploaded_file is not None:
        try:
            ...
            ax.bar(pred_dist['Attack Type'], pred_dist['Count'], color='skyblue', edgecolor='black')
            ax.set_xlabel('Attack Type')
            ax.set_ylabel('Count')
            ax.set_title('Predicted Attack Types Distribution')
...
def show_performance(models):
    ...
    ax.bar(x - 1.5*width, df_performance['Accuracy'], width, label='Accuracy', color='#1f77b4')
    ax.bar(x - 0.5*width, df_performance['Precision'], width, label='Precision', color='#ff7f0e')
    ax.bar(x + 0.5*width, df_performance['Recall'], width, label='Recall', color='#2ca02c')
    ax.bar(x + 1.5*width, df_performance['F1-Score'], width, label='F1-Score', color='#d62728')
